...
class Pokemon:
    ...
    def __init__(self, pokemon_trainer):
        ...
        self.last_feed_time = datetime.now() - timedelta(hours=1)  # Set last feed time to 1 hour ago
        self.name = None
        self.ability = None
        self.level = 1
    ...
    async def info(self):
        ...
        return f"The name of your Pokémon: {self.name}\nability: {self.ability}\nhp: {self.hp}\npower: {self.power}\nlevel: {self.level}"  # Returning the string with the Pokémon's name
    ...
    async def upgrade(self):
        if self.level >= 5:
            self.health += 10
            self.power += 5
            print(f"{self.name} has been upgraded! HP: {self.hp}, Power: {self.power}")
        else:
            print("Level must be at least 5 to upgrade.")
    async def feed(self, feed_interval = 20, hp_increase = 10 ):
        self.level += random.randint(0.1, 0.5)  # Meningkatkan level secara acak antara 0.5 hingga 1.5
...
class Wizard(Pokemon):
    ...
    async def feed(self, feed_interval = 5, hp_increase = 10 ):
        current_time = datetime.now()  
