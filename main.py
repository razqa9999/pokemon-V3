...
async def info(ctx):
    author = ctx.author.name  # Dapatkan nama pengguna yang menggunakan perintah
    if ctx.author.name in Pokemon.pokemons:  # Periksa apakah pengguna ini memiliki Pokémon 
        pokemon = Pokemon.pokemons[ctx.author.name]  # Mendapatkan objek Pokémon pengguna
        await ctx.send(await pokemon.info())  # Mengirim informasi tentang Pokémon 
    else:
        await ctx.send("Kamu belum memiliki Pokémon! Gunakan perintah !go untuk mendapatkan satu.")  # Memberi tahu pengguna bahwa mereka belum memiliki Pokémon
@bot.command()
async def upgrade(ctx):
    author = ctx.author.name  # Dapatkan nama pengguna yang menggunakan perintah
    if ctx.author.name in Pokemon.pokemons:  # Periksa apakah pengguna ini memiliki Pokémon 
        pokemon = Pokemon.pokemons[ctx.author.name]  # Mendapatkan objek Pokémon pengguna
        return await pokemon.upgrade()  # Melakukan upgrade pada Pokémon pengguna
    else:
        await ctx.send("Kamu belum memiliki Pokémon! Gunakan perintah !go untuk mendapatkan satu.")  # Memberi tahu pengguna bahwa mereka belum memiliki Pokémon
@bot.command()
async def feed(ctx):
    ...
    if ctx.author.name in Pokemon.pokemons:  # Periksa apakah pengguna ini memiliki Pokémon 
        pokemon = Pokemon.pokemons[ctx.author.name]  # Mendapatkan objek Pokémon pengguna
        result = await pokemon.feed()  # Melakukan pemberian makan dan mendapatkan hasilnya
        await ctx.send(result)  # Mengirimkan hasil pemberian makan
    else:
        await ctx.send("Kamu belum memiliki Pokémon! Gunakan perintah !go untuk mendapatkan satu.")  # Memberi tahu pengguna bahwa mereka belum memiliki Pokémon
@bot.command()
